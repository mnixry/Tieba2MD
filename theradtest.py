...
def postsGet(q, id):
    while not postsGetExit:
        ...
        if not postsWorkQueue.empty():
            data = q.get()
            tid = id
            Avalon.info('Thread %s start download page %s.' %
                        (str(tid), str(data[1])))
...
def imagesGet(q, id):
    while not imagesGetExit:
        ...
        if not imagesWorkQueue.empty():
            data = q.get()
            tid = id
            Avalon.info('Thread %s start download picture %s.' %
                        (str(tid), str((data.spilt('/')[-1]))))
...
def imagesUpload(q, id):
    while not imagesUploadExit:
        ...
        if not imagesUpWorkQueue.empty():
            data = int(q.get())
            tid = id
            Avalon.info('Thread %s start upload picture(database ID:%s).' % (
                str(tid), str(data)))
